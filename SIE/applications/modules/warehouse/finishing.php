		  ...
		  <div class="box box-primary">
			...
			switch($_GET['p']){
				...
				default:?>
					...
					if($_GET['action']=='receiving'){
						$up_date=date("d/m/Y");
						$update=$query("update tbl_raw_process SET status='1', updated_date='$up_date' where id='$_GET[id]'");
						if($update){
							...
							$newstock=$row_product['stock']+$row_raw['qty'];
							$update2=$query("update tbl_product_header SET stock='$newstock' where id='$row_product[id]'");
							if($update2){
								echo"<meta http-equiv='refresh' content='0;?m=$_GET[m]&s=$_GET[s]&l=$_GET[l]&p=list&open=$_GET[open]&msg=sks'>";
							}
						}
					...
					<div class="box-body">
						<table id="example1" class="table table-bordered table-striped">
							...
							<?php
							$nourut=0;
							$query_raw=$query("select * from tbl_raw_process where published='1' and category='$_GET[l]' order by id DESC");
							...
							//end published
								...
								if($row_raw['status']==1){
									$status="<small class='label center bg-green'>RECEIVED</small>";	
								}
								else{
									$status="<a href='?m=$_GET[m]&s=$_GET[s]&l=$_GET[l]&action=receiving&id=$row_raw[id]&open=$_GET[open]'><small class='label center bg-red'>ON PROCESS</small></a>";
								}
								?>
									<tr id="tr_table">
										<td width="30"><?=$nourut?></td>
										<td align="left"><a href="print-finishing.php?id=<?=$row_raw['id']?>" target="_blank">#<?=sprintf('%03d',$row_raw['id'])?></a><?=$correction?></td>
