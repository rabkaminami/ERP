			...
			switch($_GET['p']){
				...
				default:?>
					...
					if($_GET['action']=='payment'){
						...
						if($row_rpm['method']=='cash'){
							$update=$query("update tbl_rpm SET status='1' where id='$_GET[id]'");
							if($update){
								$query_saldo=$query("select * from tbl_saldo where id='1'");
								$row_saldo=$fetch($query_saldo);
								$newsaldo=$row_saldo['saldo']-$row_rpm['total'];
								$update=$query("update tbl_saldo SET saldo='$newsaldo' where id='$row_saldo[id]'");
							}
						}
						if($row_rpm['method']=='transfer'){
							$update=$query("update tbl_rpm SET status='1' where id='$_GET[id]'");
							if($update){
								$query_saldo=$query("select * from tbl_saldo where bank='$row_rpm[bank]'");
								$row_saldo=$fetch($query_saldo);
								$newsaldo=$row_saldo['saldo']-$row_rpm['total'];
								$update=$query("update tbl_saldo SET saldo='$newsaldo' where id='$row_saldo[id]'");
							}	
						...
						<table id="example1" class="table table-bordered table-striped">
							...
							if($approve==3){
								if($row_rpm['status']==1){
									...
									$status="<a href='?m=$_GET[m]&s=$_GET[s]&l=$_GET[l]&open=$_GET[open]&action=payment&id=$row_rpm[id]'><small class='label center bg-green'>PAYMENT</small></a>";
								}
								...
								?>
									<tr id="tr_table">
										...
										<td>
											...
											&nbsp;|&nbsp;
											<a href="print-rpm.php?id=<?=$row_rpm['id']?>" target="_blank"><i class="fa fa-print"></i> Print</a>
										</td>
									</tr>
								<?
							}?>
