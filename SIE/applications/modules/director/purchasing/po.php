					...
					if(isset($_POST['submit'])){
						if(empty($_POST['title']) || empty($_POST['supplier'])){
							...
							if($_GET['edit']==0){
								...
								$values.="'',";//published
								$values.="''";//status
								$insert=$query("insert into tbl_po_header values($values)");
								if($insert){
									for($i=1;$i<=15;$i++){
										$po_header=$id;
										$item="material$i";
				...
				default:?>
					...
					<div class="box-body">
						<table id="example1" class="table table-bordered table-striped">
							...
							<?php
							$nourut=0;
							$query_po=$query("select * from tbl_po_header where published='1' and approve1='1' and approve3='0' order by id DESC");
							...
							//end correction
								?>
									<tr id="tr_table">
										...
										<td>
											<a href="?m=<?=$_GET['m']?>&mm=<?=$_GET['mm']?>&s=<?=$_GET['s']?>&l=<?=$_GET['l']?>&p=add&edit=1&open=<?=$_GET['open']?>&id=<?=$row_po['id']?>"><img src="<?=$template?>/dist/img/edit.png"></a>
