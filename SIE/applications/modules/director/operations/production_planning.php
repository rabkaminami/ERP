		  ...
		  <div class="box box-primary">
			...
			switch($_GET['p']){
				case 'add':?>
					...
					$query_req_header=$query("select * from tbl_production_planning where id='$_GET[id]'");
				...
				default:?>
					...
					if($_GET['action']=='approve'){
						$update=$query("update tbl_production_planning SET approve2='1',correction='',correction_by='0' where id='$_GET[id]'");
					}
					if($_GET['action']=='unapprove'){
						$update=$query("update tbl_production_planning SET approve2='0' where id='$_GET[id]'");
					}
					?>
					<div class="box-body">
						<table id="example1" class="table table-bordered table-striped">
							...
							if(empty($row_pp['approve2'])){
								$approve2="<a href='?m=$_GET[m]&mm=$_GET[mm]&s=$_GET[s]&l=$_GET[l]&p=$_GET[p]&open=$_GET[open]&action=approve&id=$row_pp[id]'><img src='$template/dist/img/silang.png'></a>";	
							}
							else{
							...
							//approve status
							if($approve==3){
								$update=$query("update tbl_production_planning SET status='1' where id='$row_pp[id]'");
							...
							//end correction
								?>
									<tr id="tr_table">
										...
										<td>
											<a href="?m=<?=$_GET['m']?>&mm=<?=$_GET['mm']?>&s=<?=$_GET['s']?>&l=<?=$_GET['l']?>&p=add&open=<?=$_GET['open']?>&edit=1&id=<?=$row_pp['id']?>"><i class="fa fa-pencil"></i> Correction</a>
											&nbsp;|&nbsp;
