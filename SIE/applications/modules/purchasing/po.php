			...
			switch($_GET['p']){
				case 'add':?>
					...
					if(isset($_POST['publish'])){
						$update=$query("update tbl_po_header SET published='1', correction='$_POST[correction]' where id='$_GET[id]'");
						if($update){
							for($i=1;$i<=15;$i++){
								$ex="exist$i";
								$exist=$_POST[$ex];
				...
				default:?>
					...
					<div class="box-body">
						<table id="example1" class="table table-bordered table-striped">
							...
							//approve status
							if($approve==3){
								$update=$query("update tbl_po_header SET status='1' where id='$row_po[id]'");
