		  ...
		  <div class="box box-primary">
			...
			switch($_GET['p']){
				...
				default:?>
					...
					if($_GET['action']=='create-ro'){
						$query_sp=$query("select * from tbl_sp_header where id='$_GET[id]'");
						$row_sp=$fetch($query_sp);
						if($row_sp['approve1']==1 || $row_sp['approve2']==1 || $row_sp['approve3']==1){
							if($row_sp['correction']=='RO-CREATED'){}
							else{
								$date=date("m/d/Y")." ".date("h:i:s");
								$query_cek_ro=$query("select * from tbl_request_header order by id desc limit 1");
								$row_cek_ro=$fetch($query_cek_ro);
								if($row_cek_ro['id']==0){$ro_id=1;}
								else{$ro_id=$row_cek_ro['id']+1;}
								
								$values="'$ro_id',";//id
								$values.="'$row_sp[title]',";//title
								$values.="'4',";//department
								$values.="'$row_sp[date]',";//due_date
								$values.="'$date',";//created_date
								$values.="'',";//correction
								$values.="'',";//correction_by
								$values.="'',";//approve1
								...
								if($insert){
									$query_sp_item=$query("select * from tbl_sp_item where sp_header='$row_sp[id]'");
									while($row_sp_item=$fetch($query_sp_item)){
										$values="'',";//id
										$values.="'$ro_id',";//ro_header
										$values.="'4',";//ro_header
										$values.="'$row_sp_item[item]',";//item
										$values.="'',";//supplier
										$values.="'',";//price
										$values.="'$row_sp_item[qty]',";//qty
										$values.="'',";//total
										$values.="'$row_sp_item[unit]',";//unit
										$values.="'$row_sp_item[description]',";//description
										$values.="'$date',";//created_date
										$values.="'0'";//status
										$insert=$query("insert into tbl_request_item values($values)");
										if($insert){
											$upt=$query("update tbl_sp_header SET correction='RO-CREATED' where id='$_GET[id]'");
											if($upt){?>
						...
						<table id="example1" class="table table-bordered table-striped">
							...
							if($approve==3){
								...
								if($row_req['correction']=='RO-CREATED'){
									$status="<small class='label center bg-blue'>RO CREATED</small>";
								}else{
									$status="<a href='?m=$_GET[m]&s=$_GET[s]&l=$_GET[l]&p=$_GET[p]&open=$_GET[open]&action=create-ro&id=$row_req[id]'><small class='label center bg-green'>CREATE RO</a></small>";
								}
							...
							//end correction
								?>
									<tr id="tr_table">
										...
										<td>
											<center><a href="print-product.php?id=<?=$row_req['id']?>" target="_blank"><i class="fa fa-print"></i> Print</a></center>
										</td>
									</tr>
								<?
							}?>
