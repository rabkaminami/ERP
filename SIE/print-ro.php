...
$sl=str_replace("/","",$row_req_header['due_date']);
$barcode=$sl."".sprintf('%03d',$row_req_header['id'])
?>
<html>
<head>
	...
	<div class="page">
		...
		<div class="main">
			...
			<tr>
				...
				<th>DESCRIPTION</th>
				<th width="160"><center>TOTAL</center></th>
			...
			<?php
				...
				?>
					<tr>
						...
						<td><?=$row_spp['name']?></td>
						<td>Rp.<?=number_format($row_pr['price'],0,'','.')?></td>
						<td><?php if(empty($row_pr['qty'])){}else{?><?=$row_pr['qty']?><?}?></td>
						<td><?=$row_pr['unit']?></td>
						<td><?=$row_pr['description']?></td>
						<td>Rp. <?=number_format($row_pr['total'],0,'','.')?></td>
			...
			?>
				<?php
				$query_tot=$query("select SUM(total) as total_price from tbl_request_item where request_header='$_GET[id]'");
				$row_tot=$fetch($query_tot);
				?>
				<tr>
					<td colspan="7" align="right"><b>Total Price</b></td>
					<td>Rp. <?=number_format($row_tot['total_price'],0,'','.')?></td>
				</tr>
			</tbody>
			</table>
			<br><br><br><br>
